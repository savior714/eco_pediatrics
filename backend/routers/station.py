...
from models import MealRequest, DocumentRequest, DocumentRequestCreate
...
async def get_dashboard_data_by_token(
    ...
    if not res.data:
        raise HTTPException(
            status_code=404, detail="Invalid or inactive admission token"
        )
...
@router.get(
    "/station/pending-requests",
    response_model=List[dict],
    summary="스테이션 처리 대기 요청 목록 조회",
)
...
@router.post(
    "/documents/requests",
    response_model=DocumentRequest,
    status_code=201,
    summary="서류 발급 요청 생성",
)
async def request_document(
    ...
    if not adm_res.data:
        raise HTTPException(
            status_code=403, detail="Invalid admission ID or patient already discharged"
        )
    ...
    if existing_requests.data:
        for existing in existing_requests.data:
            if sorted(existing.get("request_items", [])) == request_items_sorted:
                logger.info(
                    f"Duplicate document request detected for admission {request.admission_id}. Skipping insertion."
                )
                return existing  # Return existing one instead of creating duplicate

    data = request.model_dump()
    response = await execute_with_retry_async(
        db.table("document_requests").insert(data)
    )
...
@router.patch(
    "/documents/requests/{request_id}",
    response_model=DocumentRequest,
    summary="서류 발급 요청 상태 변경",
)
async def update_document_request_status(
    request_id: int, status: str, db: Annotated[AsyncClient, Depends(get_supabase)]
):
...
@router.patch(
    "/meals/requests/{request_id}",
    response_model=MealRequest,
    summary="식사 요청 상태 변경",
)
async def update_meal_request_status(
    ...
    if status == "COMPLETED":
        ...
        if p_val:
            update_payload["pediatric_meal_type"] = p_val
        if g_val:
            update_payload["guardian_meal_type"] = g_val
    ...
    if not response.data:
        raise HTTPException(
            status_code=404, detail="Update failed or Request not found"
        )
    ...
    msg = {
        ...
        "data": {
            ...
            "requested_pediatric_meal_type": updated_data.get(
                "requested_pediatric_meal_type"
            ),
            "requested_guardian_meal_type": updated_data.get(
