...
async def transfer_patient(admission_id: str, req: TransferRequest, request: Request, db: Annotated[AsyncClient, Depends(get_supabase)]):
...
async def discharge_patient(admission_id: str, request: Request, db: Annotated[AsyncClient, Depends(get_supabase)]):
    ip_address = request.client.host if request.client else "127.0.0.1"
    return await admission_service.discharge_patient(db, admission_id, ip_address)

@router.post("", response_model=dict)
async def create_admission(admission: AdmissionCreate, request: Request, db: Annotated[AsyncClient, Depends(get_supabase)]):
    ip_address = request.client.host if request.client else "127.0.0.1"
    return await admission_service.create_admission(db, admission, ip_address)

@router.get("", response_model=List[dict])
async def list_admissions(response: Response, db: Annotated[AsyncClient, Depends(get_supabase)]):
...
async def get_dashboard_data_by_id(admission_id: str, db: Annotated[AsyncClient, Depends(get_supabase)]):
