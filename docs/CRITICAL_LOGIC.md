...
2. [Technical Architecture](#2-technical-architecture-기술-제약-및-패턴) — 레이어, 동기화, 모달 SSOT, DB 업데이트, 인코딩, CLI/Terminal 표준, TS 코드 품질
...
5. [Safety Guardrails](#5-safety-guardrails-수정-시-필수-점검-사항-checklist) — 수정 시 점검 체크리스트, Pre-flight & 에러 복구
...
- **Pydantic v2**: `.dict()` 호출 전면 금지. 반드시 `.model_dump()` 사용. `orm_mode = True` 대신 `model_config = ConfigDict(from_attributes=True)` 사용. `@validator` 대신 `@field_validator` 사용.
- **정적 분석 기준**: `ruff check --fix` + `mypy --ignore-missing-imports` 오류 **0개** 필수. 인라인 `# type: ignore[xxx]`는 Supabase SDK 타입 한계 등 불가피한 경우에만 허용.
...
#### 2.7.4 PowerShell 세션 초기화 표준
에이전트가 주도하는 모든 PowerShell 세션은 아래 블록을 최우선으로 실행한다.

```powershell
$OutputEncoding = [Console]::OutputEncoding = [Console]::InputEncoding = [System.Text.Encoding]::UTF8
$PSDefaultParameterValues['Out-File:Encoding'] = 'utf8'
$env:TERM = 'dumb'    # IDE 셸 통합 이스케이프 시퀀스(\e]633) 차단
$env:NO_COLOR = '1'   # 색상 코드 제거 — 순수 텍스트 파싱 유도
$ProgressPreference = 'SilentlyContinue'
Clear-Host
```

- 모든 `pwsh` 호출에 **`-NoProfile`** 플래그 필수 — 사용자 프로필 별칭 간섭 방지.

#### 2.7.5 명령어 체이닝 금지
- `;`, `&&`, `||`로 여러 명령을 한 줄에 나열하는 것을 **절대 금지**한다. 출력 버퍼가 혼재되면 에이전트는 명령 경계와 에러 범위를 구분하지 못한다.
- 반드시 **단일 원자적 명령**만 개별 실행한다.
- 출력 과다가 예상되면 `> build_log.txt 2>&1` 리다이렉션 후 `Get-Content -Tail 30` 패턴으로 읽는다.

#### 2.7.6 Terminal Noise 복구 SOP
1. **Echo Truncation** — 명령어 앞부분 잘림 감지 시 즉시 `Clear-Host` 호출.
2. **스트림 오염** — `Write-Output "=== TERMINAL_RECOVERY_MARKER ==="` 삽입으로 스트림 절단.
3. **`\e]633;` 시퀀스 감지** — 이후 모든 명령에 `pwsh -NoProfile` 접두어로 환경 격리.
4. **출력 과다/인코딩 오염** — `> build_log.txt 2>&1` 후 `Get-Content -Tail 30` 읽기.
...
### 5.2 Pre-flight Validation & Error Recovery Protocol

**Pre-flight (작업 전 선확인)**
- `Test-Path`로 설정 파일 존재 확인, `Get-Command <명령어> -ErrorAction SilentlyContinue`로 CLI 도구 가용성 확인.
  - `tsc` 실행 전: 해당 경로에 `tsconfig.json` 존재 확인
  - `npm run <script>` 실행 전: `package.json` 내 해당 스크립트 정의 확인
  - 빌드 명령 전: `node_modules` 유효성 확인, 미비 시 `npm install` 제안 우선
- **Cmdlet 파라미터 Pre-Validation**: 버전 의존적 파라미터 사용 전 `Get-Help <Cmd> -Parameter *`로 선검증한다. 확실하지 않은 옵션은 추측하지 않는다.

**Error Recovery 4단계**
1. 에러 발생 → 로컬 캐시·임시 파일 즉시 정리.
2. `git status` → 변경 범위 파악 → 필요 시 `git checkout`으로 즉시 롤백.
3. 오류를 "코드 덧대기"가 아닌 **"설계 수정"**으로 근본 해결.
4. 대규모 수정 후 반드시 `npx tsc --noEmit` 또는 `scripts/check-env.ps1` 실행으로 부수 효과 확인.
