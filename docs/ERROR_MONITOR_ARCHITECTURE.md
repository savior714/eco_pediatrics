...
## 9. 세션 생명주기: `scripts/launch_wt_dev.ps1`으로 전체 자동화

> **이 프로젝트의 현재 표준**: `eco dev` 명령 실행 시 `scripts/launch_wt_dev.ps1`이 호출되며, 에러 모니터는 `plugins/error_monitor/` 패키지 구조로 실행됩니다.

```powershell
# scripts/launch_wt_dev.ps1 — 실제 실행 명령
$wtArgs = @(
    "--title", "ErrorMonitor", "pwsh", "-NoExit", "-Command",
    "cd '$PROJECT_ROOT'; uv run python -m plugins.error_monitor --clear"
    ";", "split-pane", "-V", ...
)
...
> **이 프로젝트 표준 (eco_pediatrics)**: `plugins/error_monitor/` 패키지 구조를 사용하므로, `eco dev` 명령으로 자동 실행됩니다.

```powershell
# eco_pediatrics: eco dev 명령 실행 (launch_wt_dev.ps1 자동 호출)
eco dev

# 또는 수동 실행
uv run python -m plugins.error_monitor --clear
```
```bash
# 타 프로젝트(단일 파일 구조): 백그라운드에서
...
| **확장성** | eco_pediatrics는 `plugins/error_monitor/` DDD 3-Layer 구조(definition, repository, logic, __main__)로 이미 리팩토링 완료 (2026-03-02) |
