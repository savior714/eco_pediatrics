...
- **Tauri v2 보안 하드닝 완료**: HTTP 플러그인 권한 스코프 이슈 해결 및 데스크톱 대시보드 진입 성공
- **감사 로그 고도화**: 환자 전실/퇴원/생성 시 실제 클라이언트 IP 주소 추적 기능 도입
- **프론트엔드 최적화**: `page.tsx`와 `useStation` 훅 간의 상태 계산 로직 중복 제거 (`bed.status` 단일화)
- **백엔드 유지보수성**: `utils.py` 내 지연 임포트 제거 및 구조 개선
