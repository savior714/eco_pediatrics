...
export function PatientCard({ name, roomNumber, temperature, infusionRate, status, onCardClick }: PatientCardProps) {
    const statusStyles = {
    ...
    return (
        <Card
            className={`relative transition-all duration-300 ${statusStyles[status]} ${onCardClick ? 'cursor-pointer' : ''}`}
            onClick={onCardClick}
        >
