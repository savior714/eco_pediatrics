...
function getRoomNoteFromRow(rowData: Record<string, MealRequest>): string {
...
function getTargetMealTimeForNoteRow(rowData: Record<string, MealRequest>): string {
...
const RoomNoteInput = memo(function RoomNoteInput({ bed, rowData, onUpdate }: RoomNoteInputProps) {
    const existingNote = getRoomNoteFromRow(rowData);
        ...
        <td className="p-0 border border-slate-300 h-[32px]">
            <input
                ...
                onBlur={(e) => {
                    ...
                    const targetTime = getTargetMealTimeForNoteRow(rowData);
...
const MealRow = memo(function MealRow({ bed, rowData, onUpdate }: MealRowProps) {
...
const EMPTY_ROW: Record<string, MealRequest> = {};
...
function MealGridBase({ beds }: MealGridProps) {
    ...
    const handleUpdate = useCallback(async (
        ...
        // Use ref for current state to avoid dependency on matrix
    ...
    }, [fetchMatrix]); // Removed activeDate and matrix dependencies
                    ...
                    <tbody>
                        {patients.map((bed) => (
                            <MealRow
                                key={bed.room}
                                bed={bed}
                                rowData={matrix[bed.id || ''] || EMPTY_ROW}
