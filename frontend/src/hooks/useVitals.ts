...
import { VitalData, WsMessage, IVRecord, MealRequest, DocumentRequest, ExamScheduleItem, VitalDataResponse } from '@/types/domain';
import { useDashboardData, dashboardQueryKey, formatVitals, type DashboardResponse } from './useDashboardData';
...
export function useVitals(token: string | null | undefined, enabled: boolean = true, onDischarge?: () => void): UseVitalsReturn {
    const { data: dashboardData, isFetching, refetch, queryClient, tokenInvalidatedRef } = useDashboardData(token, enabled, onDischarge);
