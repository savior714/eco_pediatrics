...
> **주의**: 감시 대상 추가가 필요한 경우, `plugins/error_monitor/definition.py` 파일 내 `WATCH_TARGETS`와 `SOURCE_FILES`를 수정해야 합니다.
...
2. **수동 제어 (`plugins/error_monitor`)**:
   - `python -m plugins.error_monitor --clear` : 무한 루프로 로그 꼬리(tail) 감시 시작 (기존 로그 초기화)
   - `python -m plugins.error_monitor` : 기존 세션 유지하며 재시작
   - `python -m plugins.error_monitor --cleanup` : 모든 로그 및 리포트 데이터 삭제 후 종료
...
문제가 발생할 경우, 다른 LLM 모델의 도움을 받기 위해 사전 정리된 디버그 프롬프트를 사용할 수 있습니다. 해당 원본 템플릿들은 `plugins/error_monitor/resources/` 안에 백업용으로 편제되어 있습니다.
